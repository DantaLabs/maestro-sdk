...
class MaestroError(Exception):
    ...
    This is the parent class for all Maestro-specific exceptions.
    """
...
class MaestroApiError(MaestroError):
    ...
    Input:
        ...
        error_detail (str | dict | None): Additional error details from the API response.
    """

    def __init__(
        self, status_code: int, error_detail: Optional[Union[str, dict]] = None
    ) -> None:
...
class MaestroAuthError(MaestroApiError):
    ...
    Input:
        status_code (int): HTTP status code (typically 401 or 403).
        error_detail (str | dict | None): Additional authentication error details.
    """
...
class MaestroValidationError(MaestroApiError):
    ...
    Input:
        status_code (int): HTTP status code (typically 422).
        error_detail (dict | List | str | None): Validation error details from FastAPI.
    """

    def __init__(
        self, status_code: int, error_detail: Optional[Union[dict, List, str]] = None
    ) -> None:
        ...
        if isinstance(error_detail, dict) and "detail" in error_detail:
            ...
            error_detail, list
        ):  # Sometimes FastAPI might return just the list
        ...
        if self.validation_errors:
            ...
            try:
                formatted_errors = ", ".join(
                    [
                        f"{e.get('loc', ['unknown'])[-1] if e.get('loc') else 'field'}: {e.get('msg', 'N/A')}"
